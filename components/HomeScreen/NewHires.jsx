...
const NewHiresCard = forwardRef((props, ref) => {
    ...
    const getData = async () => {
        setLoading(true);
        try {
            const response = await Http.get(`/newHires`)
    ...
    return (
        <View className="mb-6 bg-white dark:bg-black-100 p-4 rounded-xl shadow-md w-full max-w-sm mx-auto">
            <View className="m-4 flex flex-row">
                <Image source={require('../../assets/icons/user.png')} className='w-12 h-12 shadow-lg ' />
                <Text className="ml-8 mt-4 text-xl font-bold dark:text-gray-100">New Hires</Text>
            </View>
            <View>
                {loading ? (
                    ...
                    <>
                        {!data.length && (
                            <Text className="text-center text-gray-400 font-semibold p-6 dark:text-gray-100">No New Joiness in past 15 days!</Text>
                        )}
                        {displayedItems?.map((item, index) => (
                            ...
                            >
                                ...
                                <View className="flex-col">
                                    ...
                                    <Text className="text-gray-500 dark:text-gray-100">{item.designation_name}</Text>
                                </View>
                            </Pressable>
                        ))}
                    </>
