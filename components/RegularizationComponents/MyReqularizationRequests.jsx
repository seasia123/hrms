...
const MyRegularisationRequests = () => {
    ...
    const getData = async () => {
        ...
        try {
            const response = await Http.post("/getMyRegularizationList", payload);
            if (response.ok) {
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <View className="flex-row items-center justify-between p-4 border-b border-gray-300 dark:border-gray-700">
                <TouchableOpacity className="flex-row items-center" onPress={() => router.push(`/regularization-detail/${item.id}?approval=false`)}>
                    {avatarExists ? (
                        <Image
                    ...
                    <View className="flex-1 ml-4">
                        <Text className="font-semibold dark:text-gray-100" numberOfLines={1} ellipsizeMode="tail">{item.employee_id}&nbsp;-&nbsp;{user.name}</Text>
...
export default MyRegularisationRequests;